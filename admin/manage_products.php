...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'update') {
        ...
        $stmt = $conn->prepare("UPDATE products SET name=?, sku=?, category=?, unit_price=?, current_stock=?, min_stock_level=?, lead_time_days=? WHERE id=?");
        $stmt->bind_param("sssdiiii", $name, $sku, $category, $price, $stock, $min_stock, $lead_time, $pid);
        if ($stmt->execute()) {
...
$products = $conn->query("SELECT p.*, u.name as creator FROM products p LEFT JOIN users u ON p.created_by = u.id ORDER BY p.created_at DESC");
...
<div class="page-header">
    ...
    <p>Add, edit, and remove inventory products</p>
</div>
...
<div class="form-card">
    <h3 style="margin-bottom:20px"><?= $editing ? 'Edit Product' : 'Add New Product' ?></h3>
    <form method="POST">
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                <label>SKU</label>
                <input type="text" name="sku" required value="<?= htmlspecialchars($editing['sku'] ?? '') ?>" placeholder="e.g. PP-101">
            </div>
        </div>
        <div class="form-row">
            <div class="form-group">
                <label>Category</label>
                <input type="text" name="category" value="<?= htmlspecialchars($editing['category'] ?? '') ?>" placeholder="e.g. Office Supplies">
            </div>
            <div class="form-group">
                <label>Unit Price ($)</label>
                <input type="number" name="unit_price" step="0.01" value="<?= $editing['unit_price'] ?? '0.00' ?>">
            </div>
        </div>
        <div class="form-row">
            <div class="form-group">
                <label>Current Stock</label>
                <input type="number" name="current_stock" value="<?= $editing['current_stock'] ?? '0' ?>">
            </div>
            <div class="form-group">
    ...
    <table class="data-table" id="productsTable">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="mono"><?= htmlspecialchars($p['sku']) ?></td>
                ...
                <td class="mono text-muted"><?= $p['min_stock_level'] ?></td>
                <td class="mono text-muted"><?= $p['lead_time_days'] ?>d</td>
                <td>
                    <div class="btn-group">
                        <a href="?edit=<?= $p['id'] ?>" class="btn btn-secondary btn-sm">Edit</a>
                        <form method="POST" style="display:inline" onsubmit="return confirmDelete('Delete this product and all its data?')">
