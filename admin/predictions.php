...
<div class="page-header">
    ...
    <p>Intelligent stock forecasting and reorder recommendations</p>
</div>
...
<div class="stats-grid">
    <div class="stat-card red">
        ...
        <div class="stat-sub">&lt; 7 days stock</div>
    </div>
    <div class="stat-card yellow">
        <div class="stat-label">Warning</div>
        <div class="stat-value"><?= $stats['warning'] ?></div>
        <div class="stat-sub">7-30 days stock</div>
    </div>
    <div class="stat-card green">
        <div class="stat-label">Healthy</div>
        <div class="stat-value"><?= $stats['healthy'] ?></div>
        <div class="stat-sub">30+ days stock</div>
    </div>
    ...
    <div class="pred-card" data-status="<?= $pred['status'] ?>">
        <div class="pred-card-header">
            <div>
                ...
                <div class="pred-sku"><?= htmlspecialchars($pred['product']['sku']) ?> &middot; <?= htmlspecialchars($pred['product']['category']) ?></div>
            </div>
            <div class="status-dot <?= $pred['status'] ?>"></div>
        </div>
        <div class="pred-stats">
            <div class="pred-stat">
                <div class="pred-stat-label">Current Stock</div>
                <div class="pred-stat-value <?= $pred['status'] ?>"><?= $pred['product']['current_stock'] ?></div>
            </div>
            <div class="pred-stat">
                <div class="pred-stat-label">Avg Daily Use</div>
                <div class="pred-stat-value"><?= $pred['avg_daily_consumption'] ?></div>
            </div>
            <div class="pred-stat">
                <div class="pred-stat-label">Days Until Stockout</div>
            ...
            <div class="pred-stat">
                ...
                <div class="pred-stat-value" style="font-size:14px"><?= $pred['days_until_stockout'] >= 999 ? 'N/A' : date('M d', strtotime($pred['predicted_stockout_date'])) ?></div>
            </div>
            <div class="pred-stat">
                <div class="pred-stat-label">Suggested Order</div>
